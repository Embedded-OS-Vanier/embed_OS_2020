/**
 ...
 Author		Date		Version
 SH			Dec. 2019	V1.0.0
 SH			Dec. 2019	V1.0.1	Cleanup, add SLAVE_ADDRESS macro
 SH         Jan. 2020   v1.0.2  Add a mutex to I2C1_set_data()
...
static uint8_t i2c_slave_data[SLAVE_DATA_SIZE] =
            {
...
/**
  Prototype:        void I2C1_set_data(uint8_t data, uint8_t index)
  Input:            data to write, index to indicate location..
  Output:           returns 0 on success otherwise returns 0
  Description:      I2C1_set_data sets the data in the i2c_slave_data buffer.
  Comment:          
  Usage:            void I2C1_set_data(int data, int index)
                    data : value to write into the i2c_slave_data buffer
                    index: the index or position where the data is written
*/
 int I2C1_set_data(uint8_t data, uint8_t index){
     int ret =0;
     xSemaphoreTake(xMutex, portMAX_DELAY);
     if(index< SLAVE_DATA_SIZE){
         i2c_slave_data[index]=data;
         ret =0;
     }
     else ret =-1;
...
void I2C1_Initialize(void)
{
    ...
    I2C1STAT = 0x00;
    // I2CADD 170; 
    I2C1_SlaveAddressSet(SLAVE_ADDRESS);
    // AMSK 0; 
    ...
    */
    ...
    switch (status)
    {
        case I2C1_SLAVE_TRANSMIT_REQUEST_DETECTED:
            ...
            I2C1_ReadPointerSet(&i2c_slave_data[address++]);
            if(address >= SLAVE_DATA_SIZE) {
                address = 0;
            }
            break;
        ...
        case I2C1_SLAVE_RECEIVED_DATA_DETECTED:
            ...
            if (addressState == true)
            {
                ...
                if(address >= SLAVE_DATA_SIZE) {
                    address = SLAVE_DATA_SIZE;
                }
            }
            else // if (addressState == false)
            {
                // set the memory with the received data
                i2c_slave_data[address++] = i2c1_slaveWriteData;
                if(address >= SLAVE_DATA_SIZE) {
                    address = 0;
                }
            }
